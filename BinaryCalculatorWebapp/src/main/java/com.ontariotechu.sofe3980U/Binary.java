...
public class Binary {
    ...
    /**
     * Binary OR operation between two binary variables.
     * 
    ...
    public static Binary or(Binary num1, Binary num2) {
        // Adjusting lengths by padding with zeros
        int maxLength = Math.max(num1.number.length(), num2.number.length());
        String operand1 = String.format("%" + maxLength + "s", num1.number).replace(' ', '0');
        String operand2 = String.format("%" + maxLength + "s", num2.number).replace(' ', '0');
    ...
    /**
     * Binary Multiplication operation between two binary variables.
     * 
     * @param num1 The first operand
     * @param num2 The second operand
     * @return A binary variable with a value of <i>num1 * num2</i>.
    ...
    public static Binary multiply(Binary num1, Binary num2) {
        ...
        for (int i = multiplier.length() - 1; i >= 0; i--) {
            if (multiplier.charAt(i) == '1') {
                ...
                String shiftedValue = multiplicand + "0".repeat(multiplier.length() - 1 - i);
                product = add(product, new Binary(shiftedValue));
            }
        }
